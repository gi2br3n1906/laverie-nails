<x-layouts.app title="How to Measure Your Nails">
    ...
    <section class="mx-auto mt-20 max-w-4xl space-y-20" aria-label="Measurement instructions">
        <article class="flex flex-col items-center text-center">
            ...
            <div class="mt-8 grid w-full grid-cols-3 gap-2 sm:gap-3 md:gap-4" data-guidance-image-grid="measure-1">
                @for ($image = 1; $image <= 3; $image++)
                    <img class="aspect-[3/4] h-auto w-full rounded-[1.5rem] border border-[#92A1B5]/40 bg-white object-contain shadow-xl shadow-[#0C1C39]/5" src="{{ asset('images/measure-1.svg') }}" alt="Measure 1 illustration {{ $image }}: place a millimeter ruler across the widest part of the nail plate">
                @endfor
            </div>
        ...
        <article class="flex flex-col items-center border-t border-[#92A1B5]/30 pt-20 text-center">
            ...
            <div class="mt-8 grid w-full grid-cols-3 gap-2 sm:gap-3 md:gap-4" data-guidance-image-grid="measure-2">
                @for ($image = 1; $image <= 3; $image++)
                    <img class="aspect-[3/4] h-auto w-full rounded-[1.5rem] border border-[#92A1B5]/40 bg-white object-contain shadow-xl shadow-[#0C1C39]/5" src="{{ asset('images/measure-2.svg') }}" alt="Measure 2 illustration {{ $image }}: record the nail width in millimeters for all five fingers">
                @endfor
            </div>
