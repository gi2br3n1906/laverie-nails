<x-layouts.app title="Pengaturan Akun">
    ...
    <div class="mt-8 grid gap-8 lg:grid-cols-2">
        ...
        <section class="rounded-[2rem] border border-[#92A1B5]/40 bg-white p-6 shadow-sm sm:p-8 lg:col-span-2" aria-labelledby="address-heading">
            ...
            @unless ($logisticsAvailable)
                <div class="mt-5 rounded-2xl border border-[#92A1B5]/50 bg-[#EAF0F6] px-4 py-3 text-sm leading-6 text-[#0C1C39]" role="status">
                    <span class="font-semibold">Layanan wilayah belum dapat dimuat.</span> Informasi pribadi dan password tetap dapat diperbarui. Konfigurasikan RajaOngkir untuk mengubah alamat pengiriman.
                </div>
            @endunless
            <form class="mt-6 grid gap-5 md:grid-cols-2" action="{{ route('profile.address.update') }}" method="POST" data-profile-address-form data-cities-url="{{ route('profile.logistics.cities') }}">
                ...
                <label class="block text-sm font-semibold text-[#0C1C39]">Provinsi
                    <select class="mt-2 w-full rounded-xl border border-[#92A1B5]/60 bg-[#F8FAFC] px-4 py-3 outline-none focus:border-[#0C1C39] focus:ring-4 focus:ring-[#92A1B5]/20 disabled:cursor-not-allowed disabled:bg-stone-100" name="province_id" data-profile-province @disabled(! $logisticsAvailable) required>
                ...
                <div class="md:col-span-2"><button class="inline-flex min-h-12 items-center justify-center rounded-full bg-[#0C1C39] px-7 text-xs font-semibold uppercase tracking-[0.14em] text-white transition hover:bg-[#192B48] disabled:cursor-not-allowed disabled:opacity-60" type="submit" @disabled(! $logisticsAvailable)>Simpan alamat</button></div>
            </form>
        </section>
    </div>
</x-layouts.app>