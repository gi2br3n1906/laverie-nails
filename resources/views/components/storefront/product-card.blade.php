...
<article class="group min-w-0" data-homepage-product-card>
    <div class="relative overflow-hidden rounded-[1.5rem] bg-[#f1ebe7]">
        ...
        <span class="absolute left-3 top-3 rounded-full bg-[#f6d7d8] px-3 py-1.5 text-[0.65rem] font-semibold uppercase tracking-[0.1em] text-[#8e3439] sm:left-4 sm:top-4">{{ $badge }}</span>
    ...
    <div class="px-1 pt-4">
        ...
        <a href="{{ route('products.show', $catalog) }}"><h3 class="mt-2 font-serif text-xl leading-tight tracking-[-0.02em] text-stone-950 sm:text-2xl">{{ $catalog->title }}</h3></a>
