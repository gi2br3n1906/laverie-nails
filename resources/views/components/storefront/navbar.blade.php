<header class="sticky top-0 z-50 border-b border-[#92A1B5]/40 bg-white/95 text-[#0C1C39] backdrop-blur-xl" data-homepage-navbar>
    <nav class="relative mx-auto grid h-16 max-w-screen-2xl grid-cols-3 items-center px-4 sm:h-20 sm:px-8 lg:px-12" aria-label="Navigasi toko">
        <div class="flex items-center justify-start">
            <details class="group relative">
                <summary class="grid size-10 cursor-pointer list-none place-items-center rounded-full transition hover:bg-stone-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0C1C39]" aria-label="Buka menu">
                ...
                <div class="absolute left-0 top-12 w-64 rounded-2xl border border-[#92A1B5]/40 bg-white p-3 shadow-2xl shadow-[#0C1C39]/10 sm:top-14">
            ...
            <a class="ml-2 hidden text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-stone-600 transition hover:text-[#0C1C39] lg:inline" href="{{ route('products.index') }}">Shop</a>
        </div>

        <a class="justify-self-center whitespace-nowrap font-logo text-2xl tracking-[-0.04em] text-[#0C1C39] sm:text-3xl" href="{{ route('home') }}">Laverie Nails</a>
        ...
        <div class="flex items-center justify-end gap-0.5 sm:gap-1">
            ...
            <a class="relative grid size-10 place-items-center rounded-full transition hover:bg-stone-100" href="{{ route('products.index') }}" aria-label="Tas belanja">
                ...
                <span class="absolute right-1 top-1 grid size-4 place-items-center rounded-full bg-[#0C1C39] text-[0.55rem] font-bold text-white">0</span>
            </a>
        </div>
    </nav>
</header>